...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) 
{
    ...
    for (float t = 0; t <= 1; t = t + 0.001) {
        ...
        //point赋予了颜色后  九宫格中心就有了颜色，同时根据距离平滑到其他像素
        
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                //计算距离权重  
                float dis = std::sqrt(std::pow(point.y - int(point.y + j) - 0.5, 2) + std::pow(point.x - int(point.x + i) - 0.5, 2));//距离
                float weight =  1 - dis * std::sqrt(2) / 3;
                float color = weight * 255;

                //更新了point 九宫格也会移动 防止新的point 平滑颜色覆盖了大权重的或者之前的point 所以需要判断是否是最大，  
